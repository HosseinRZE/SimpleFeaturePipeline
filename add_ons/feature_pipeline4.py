...
class FeaturePipeline:
    ...
    def _normalize_single(self, df, norm_cfg, fit, dict_name):
        ...
        for col, method in norm_cfg.items():
            ...
            elif method in (None, "none"): continue
            else: raise ValueError(f"Unknown normalization method: {method}")
    ...
    def fit_transform(self, df, save_path=None):
        df_out = df.copy()
        # MODIFIED: Handle steps that may return a tuple
        for step in self.steps:
            result = step(df_out)
            if isinstance(result, tuple):
                df_out, _ = result  # Unpack the dataframe, ignore indices
            else:
                df_out = result
