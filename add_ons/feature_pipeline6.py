...
class FeaturePipeline:
    def __init__(self, steps=None, per_window_flags=None, norm_methods=None, window_norms=None, transformations= None,afterburner_steps = None):
        self.steps = steps or []
        ...
        # Store afterburner steps
        self.afterburner_steps =afterburner_steps or []
    ...
    def apply_afterburners(self, df_data, df_labels):
        """
        General-purpose afterburner application.
        Applies afterburner steps to feature and label dataframes separately.
        """
        if not self.afterburner_steps:
            return df_data, df_labels, {}

        # Keep track of original columns to split back correctly
        feature_cols = df_data.columns.tolist()
        label_cols = df_labels.columns.tolist()

        all_info = {}
        if not hasattr(self, "afterburner_info"):
            self.afterburner_info = {}

        # This loop now works for any afterburner function
        for afterburner_func in self.afterburner_steps:
            # ✅ Call the afterburner function with df_data and df_labels separately
            result = afterburner_func(df_data, df_labels)  # Pass separately

            # Standard pipeline result unpacking
            if isinstance(result, tuple):
                    df_data, df_labels, info = result  # df_data, df_labels, info
            if isinstance(info, dict):
                all_info.update(info)
                self.afterburner_info.update(info)
            else:
                raise ValueError(f"Unexpected afterburner return format: {result}")

        # Return the updated feature and label DataFrames directly
        return df_data, df_labels, all_info


    def add_afterburners(self, afterburner_steps):
        """Method to add afterburner steps."""
        self.afterburner_steps = afterburner_steps
