...
def preprocess_sequences_csv_multilines(
    ...
    # Apply global feature steps again to generate the final dataframe and collect invalid indices
    # NOTE: Your FeaturePipeline's .fit() method already applies these steps internally.+    # This second pass is retained from your original code structure.
    # The key change is handling the tuple output to collect bad indices.
    if feature_pipeline is not None:
        for step, per_window in zip(feature_pipeline.steps, feature_pipeline.per_window_flags):
            if not per_window:
                result = step(df_main)
                if isinstance(result, tuple):
                    df_main, bad_indices = result
                    invalid_row_indices.update(bad_indices)
                else:
                    df_main = result
    ...
    for _, row in df_labels.iterrows():
        # Check if the sequence's index range contains any rows marked as invalid
        seq_indices = range(row['startIndex'], row['endIndex'] + 1)
        if not invalid_row_indices.isdisjoint(seq_indices):
