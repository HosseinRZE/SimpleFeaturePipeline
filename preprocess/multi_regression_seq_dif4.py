...
def preprocess_sequences_csv_multilines(
    ...
    if feature_pipeline is not None and feature_pipeline.afterburner_steps:
        # 1. Gather all data into arrays
        all_features_arr = np.vstack([d['main'] for d in X_dicts_list])
        all_labels_arr = np.vstack(y_list)
        all_features_arr = np.nan_to_num(all_features_arr, nan=0.0)        
        # 2. ✅ CREATE MASK: Remember where the real labels are BEFORE scaling
        label_mask = (all_labels_arr != 0)
        # 3. Create DataFrames for the afterburner
        df_all_features = pd.DataFrame(all_features_arr, columns=feature_cols)
        df_all_labels = pd.DataFrame(all_labels_arr, columns=lineprice_cols)
        # 4. Call the afterburner ONCE on the complete dataset
        df_scaled_features, df_scaled_labels, _ = feature_pipeline.apply_afterburners(
            df_all_features, df_all_labels
        )

        # 5. ✅ RESTORE PADDING: Use the mask to set padded values back to zero
        scaled_labels_arr = df_scaled_labels.values
        scaled_labels_arr[~label_mask] = 0.0

        # 6. Rebuild your lists with the newly scaled and corrected data
        scaled_features_arr = df_scaled_features.values
        
        new_X_dicts_list = []
        current_pos = 0
        for length in x_lengths:
            sample_features = scaled_features_arr[current_pos : current_pos + length]
            new_X_dicts_list.append({'main': sample_features})
            current_pos += length
        
        y_list = [row for row in scaled_labels_arr]
        X_dicts_list = new_X_dicts_list
